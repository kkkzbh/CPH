...
2. 打开右上角的 **开发者模式**。
3. 点击 **加载已解压的扩展程序**。
4. 如果已经运行过 `./gradlew buildPlugin`，选择 `build/distributions/cph-target-runner-browser-1.0.3/`；如果是在当前源码仓库里开发，选择 `browser-extension/cph-target-runner/`。
...
如果你在 `Settings / Tools / CPH Target Runner` 中修改了 CPH 监听端口，也需要把新端口告诉浏览器扩展：
