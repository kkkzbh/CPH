...
- CMake Application 和 C/C++ File 样例支持 Debug，插件会把当前样例输入接到 CLion 原生调试器。
...
当前实现的 Debug 支持 **CMake Application** 和 **C/C++ File**。插件会创建临时 Debug 配置，把当前样例输入写入临时文件，并通过 CLion 的输入重定向接入原生调试器。

如果当前运行配置不是 CMake Application 或 C/C++ File，Debug 会提示不支持。
...
Debug 支持 CMake Application 和 C/C++ File。若按钮不可用，先确认当前选中的是这两类运行配置之一，并且当前样例已被选中。
`,
  },
];
